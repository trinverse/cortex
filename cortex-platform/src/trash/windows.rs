...
impl TrashOperations for WindowsTrash {
    fn move_to_trash(&self, path: &Path) -> Result<()> {
        ...
        if result != 0 {
            return Err(anyhow::anyhow!("Failed to move to recycle bin: error code {}", result));
        }
    ...
    fn restore_from_trash(&self, _path: &Path) -> Result<()> {
        ...
        Err(anyhow::anyhow!("Restore from recycle bin is not yet implemented on Windows"))
    }
    ...
    fn empty_trash(&self) -> Result<()> {
        ...
        if result != 0 {
            return Err(anyhow::anyhow!("Failed to empty recycle bin: error code {}", result));
        }
