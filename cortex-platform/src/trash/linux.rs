...
impl LinuxTrash {
    ...
    fn write_trash_info(&self, original_path: &Path, trash_name: &str) -> Result<()> {
        let info_path = self.trash_dir.join("info").join(format!("{}.trashinfo", trash_name));
        ...
        fs::write(&info_path, info_content)
            .context("Failed to write trash info file")?;
...
impl TrashOperations for LinuxTrash {
    ...
    fn restore_from_trash(&self, trash_path: &Path) -> Result<()> {
        ...
        let info_path = self.trash_dir.join("info").join(format!("{}.trashinfo", file_name));
    ...
    fn list_trash_contents(&self) -> Result<Vec<TrashItem>> {
        ...
        for entry in fs::read_dir(&info_dir)? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("trashinfo") {
                ...
                let deletion_date = chrono::DateTime::parse_from_rfc3339(&format!("{}+00:00", deletion_date_str))
                    .ok()
                ...
                let size = fs::metadata(&trash_file_path)
                    .map(|m| m.len())
                    .unwrap_or(0);
