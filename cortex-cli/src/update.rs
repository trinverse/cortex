use anyhow::Result;
use cortex_updater::{AutoUpdater, UpdateConfig, UpdateInfo, UpdateStatus, UpdateChannel};
use semver::Version;
use std::sync::Arc;
...
impl UpdateManager {
    ...
    pub async fn download_update(&self, update_info: UpdateInfo) -> Result<()> {
        ...
        let download_path = updater.download_update(
            &update_info,
    ...
    pub async fn install_update(&self, update_info: UpdateInfo) -> Result<()> {
        ...
        let updater = self.updater.read().await;
        let update_path = updater.download_update(
            &update_info,
