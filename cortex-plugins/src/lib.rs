...
pub enum PluginEvent {
    ...
    FileOperation { operation: String, source: PathBuf, destination: Option<PathBuf> },
    PanelFocused { panel: String },
...
impl UserData for PluginContext {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        methods.add_method("get_current_file", |_, this, ()| {
            Ok(this.current_file.as_ref().map(|p| p.to_string_lossy().to_string()))
        });
...
impl UserData for CortexAPI {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method("list_directory", |_, _, path: String| {
            match std::fs::read_dir(&path) {
                Ok(entries) => {
                    let files: Vec<String> = entries
                        .filter_map(|entry| {
                            entry.ok()?.file_name().to_str().map(|s| s.to_string())
                        })
                        .collect();
                    Ok(files)
                },
        ...
        methods.add_method("get_env", |_, _, var: String| {
...
pub trait Plugin: Send + Sync {
    ...
    async fn execute(&self, command: &str, args: Vec<String>, context: PluginContext) -> Result<String>;
...
impl LuaPlugin {
    fn serialize_event_data(&self, event: &PluginEvent) -> Result<HashMap<String, String>> {
        ...
        match event {
            ...
            PluginEvent::FileOperation { operation, source, destination } => {
                data.insert("operation".to_string(), operation.clone());
                data.insert("source".to_string(), source.to_string_lossy().to_string());
                if let Some(dest) = destination {
                    data.insert("destination".to_string(), dest.to_string_lossy().to_string());
                }
            },
...
impl PluginManager {
    ...
    pub async fn load_plugin(&mut self, plugin: Box<dyn Plugin>) -> Result<()> {
        ...
        for command in &plugin_info.commands {
            self.plugin_commands.insert(command.clone(), plugin_info.name.clone());
        }
    ...
    pub async fn initialize_all(&mut self, base_context: PluginContext) -> Result<()> {
        for plugin in &mut self.plugins {
            let api = CortexAPI { context: base_context.clone() };
            plugin.initialize(api).await?;
        }
        Ok(())
    }
    
    pub async fn execute_command(&self, command: &str, args: Vec<String>, context: PluginContext) -> Result<String> {
