...
impl CompressedFileEntry {
    ...
    pub fn from_entry(
        ...
        let modified = entry.modified.map(|dt| dt.timestamp() as u64).unwrap_or(0);
...
impl MemoryManager {
    ...
    pub fn decompress_entries(
        &self,
        compressed: &[CompressedFileEntry],
    ...
    fn estimate_memory_usage(
        string_stats: &StringPoolStats,
        path_size: usize,
        entries_in_use: usize,
    ) -> usize {
...
impl<T> BatchProcessor<T> {
        ...
        F: Fn(&[T]) -> Vec<R> + Send + Sync + Clone + 'static,
    {
        ...
        for chunk in items.chunks(self.batch_size) {
            ...
            let handle = task::spawn(async move { proc(&chunk) });
