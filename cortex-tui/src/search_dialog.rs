use cortex_core::{SearchCriteria, SearchResult, SearchType};
use ratatui::{
...
impl SearchDialog {
    ...
    fn render_setup(&self, frame: &mut Frame) {
        ...
        let pattern = Paragraph::new(pattern_text).style(if self.criteria.pattern.is_empty() {
        ...
        let dir_text = Paragraph::new("Current directory").style(Style::default().fg(Color::White));
        ...
        let ext = Paragraph::new(ext_text).style(if self.criteria.file_extensions.is_empty() {
            Style::default().fg(Color::DarkGray)
        } else {
            Style::default().fg(Color::White)
        });
        ...
        // Help
        let help_text =
            " Tab: Next field | Space: Toggle option | Enter: Start search | ESC: Cancel ";
    ...
    fn render_searching(&self, frame: &mut Frame) {
        ...
        if let Some(ref progress) = self.search_progress {
            // Current path
            let path_text = format!("Searching: {}", progress.current_path.display());
            let path = Paragraph::new(path_text).style(Style::default().fg(Color::White));
            ...
            if !result.matches.is_empty() {
                if let Some(first_match) = result.matches.first() {
                    if let Some(ref content) = first_match.line_content {
                        let preview_text =
                            format!("Line {}: {}", first_match.line_number.unwrap_or(0), content);
                        let preview =
                            Paragraph::new(preview_text).style(Style::default().fg(Color::Yellow));
