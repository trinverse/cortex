...
impl EditorDialog {
    ...
    pub fn render(&mut self, frame: &mut Frame) {
        ...
        let title = format!(" Editing: {} {} ", 
            self.editor.title, 
            if self.editor.modified { "[Modified]" } else { "" }
        );
        ...
        for i in 0..height {
            ...
            if let Some(ref term) = self.editor.search_term {
                if let Some(pos) = line.to_lowercase().find(&term.to_lowercase()) {
                    ...
                    spans.push(Span::styled(matched, 
        ...
        if !self.search_mode && !self.replace_mode {
            let cursor_x = inner.x + line_number_width as u16 + 
    ...
    fn apply_syntax_highlighting(&self, line: &str) -> Vec<Span> {
        ...
        let ext = self.editor.path.extension()
            .and_then(|e| e.to_str())
            .unwrap_or("");
        ...
        match ext {
            "rs" => {
                ...
                } else if line.contains("fn ") || line.contains("impl ") || 
                ...
                } else if line.contains("pub ") || line.contains("let ") || 
                          line.contains("const ") || line.contains("mut ") {
                    spans.push(Span::styled(line.to_string(), 
