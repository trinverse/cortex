...
impl PluginDialog {
    ...
    pub fn is_selected_plugin_enabled(&self) -> bool {
        self.plugin_states.get(self.selected_index).copied().unwrap_or(true)
    }
    ...
    fn render_plugin_list(&self, frame: &mut Frame, area: Rect) {
        ...
        if self.plugins.is_empty() {
            ...
            let items: Vec<ListItem> = self.plugins
                .iter()
                .enumerate()
                .map(|(idx, plugin)| {
        ...
        if let Some(plugin) = self.get_selected_plugin() {
            ...
            let mut details_text = vec![
                ...
                Line::from(vec![
                    ...
                    Span::styled(plugin.description.clone(), Style::default().fg(Color::White)),
                ]),
                Line::from(vec![
                    Span::styled("Min Cortex Version: ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                    Span::styled(plugin.min_cortex_version.clone(), Style::default().fg(Color::White)),
                ]),
                Line::from(""),
            ];
            ...
            if !plugin.event_hooks.is_empty() {
                details_text.push(Line::from(vec![
                    Span::styled("Event Hooks: ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                ]));
