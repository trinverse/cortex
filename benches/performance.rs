...
use cortex_core::{
    FileSystem, DirectoryCache, CacheConfig, VirtualScroller, VirtualScrollConfig,
    StringPool, MemoryManager, ShortcutManager
};
use std::path::Path;
    ...
    for (path, size) in test_dirs {
        if Path::new(path).exists() {
            group.bench_with_input(
                ...
                |b, &path| {
                    b.iter(|| {
                        FileSystem::list_directory(Path::new(path), false)
                    });
                },
            );
        }
...
fn bench_cache_operations(c: &mut Criterion) {
    ...
    group.bench_function("cache_put", |b| {
        b.iter(|| {
            cache.put(black_box(test_path), black_box(entries.clone())).ok();
        });
    });
    ...
    group.bench_function("cache_get_miss", |b| {
        b.iter(|| {
            cache.get(black_box(missing_path))
        });
    });
...
fn bench_string_pool(c: &mut Criterion) {
    ...
    group.bench_function("intern_new", |b| {
        b.iter(|| {
            pool.intern(black_box("unique_string.txt"))
        });
    });
    ...
    group.bench_function("intern_existing", |b| {
        b.iter(|| {
            pool.intern(black_box("file.txt"))
        });
    });
...
fn bench_memory_manager(c: &mut Criterion) {
    ...
    group.bench_function("compress_entries", |b| {
        b.iter(|| {
            manager.compress_entries(black_box(&entries))
        });
    });
    ...
    group.bench_function("decompress_entries", |b| {
        b.iter(|| {
            manager.decompress_entries(black_box(&compressed))
        });
    });
...
fn bench_shortcuts(c: &mut Criterion) {
    ...
    for (code, modifiers) in test_keys {
        group.bench_with_input(
            ...
            |b, &(code, modifiers)| {
                b.iter(|| {
                    manager.get_action(black_box(code), black_box(modifiers))
                });
            },
        );
    }
...
fn bench_file_operations(c: &mut Criterion) {
    ...
    group.bench_function("read_file_metadata", |b| {
        b.iter(|| {
            fs::metadata(black_box(&test_file))
        });
    });
